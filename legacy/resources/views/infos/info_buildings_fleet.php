<br>
<div id="content" role="main">
    <table width="519">
        <tbody>
            <tr>
                <td class="c" colspan="2">{in_title_head} {element_typ}</td>
            </tr><tr>
                <th scope="row">{in_name}</th>
                <th role="cell">{name}</th>
            </tr><tr>
                <th role="cell" colspan="2">
                    <table>
                        <tbody>
                            <tr>
                                <td><img src="{dpath}elements/{image}.gif" align="top" border="0" height="120" width="120" alt=""/></td>
                                <td>{description}<br><br>{rf_info_fr}{rf_info_to}</td>
                            </tr>
                        </tbody>
                    </table>
                </th>
            </tr><tr>
                <th scope="row">{in_struct_pt}</th>
                <th role="cell">{hull_pt}</th>
            </tr><tr>
                <th scope="row">{in_shield_pt}</th>
                <th role="cell">{shield_pt}</th>
            </tr><tr>
                <th scope="row">{in_attack_pt}</th>
                <th role="cell">{attack_pt}</th>
            </tr><tr>
                <th scope="row">{in_base_speed}</th>
                <th role="cell">{base_speed} {upd_speed}</th>
            </tr><tr>
                <th scope="row">{in_capacity}</th>
                <th role="cell">{capacity_pt}&nbsp;{in_units}</th>
            </tr><tr>
                <th scope="row">{in_consumption}</th>
                <th role="cell">{base_conso} {upd_conso}</th>
            </tr>
        </tbody>
    </table>
</div>