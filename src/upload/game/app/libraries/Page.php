...
class Page
{
    ...
    {
        $lang = $this->langs->loadLang(['global', 'navigation', 'officier'], true);
    ...
    {
        $lang = $this->langs->loadLang('menu', true);
        ...
        $pages = [
            ...
            ['signout', $lang->line('lm_logout'), '', 'FFF', '', '2', ''],
        ...
        foreach ($pages as $key => $data) {
            ...
            // BUILD URL
            if ($data[2] != '') {
                $link = 'index.php?page=' . $data[0] . '&' . $data[2];
            } else {
        ...
        $parse['user_name'] = UrlHelper::setUrl('index.php?page=preferences', $this->current_user['user_name']);
        ...
        $parse['admin_link'] = (($this->current_user['user_authlevel'] > 0) ?
            "<tr><td><div align=\"center\"><a href=\"../admin\" target=\"_blank\">
            <font color=\"lime\">" . $lang->line('lm_administration') . "</font></a></div></td></tr>" : "");
        $parse['servername'] = Functions::readConfig('game_name');
        $parse['changelog'] = UrlHelper::setUrl('index.php?page=changelog', SYSTEM_VERSION);
        $parse['version'] = SYSTEM_VERSION;
    ...
    private function buildPlanetList()
    {
        ...
        if ($user_planets) {
            while ($planets = $db->fetchArray($user_planets)) {
                ...
                $list .= "value=\"index.php?page=" . $page . "&gid=" .
