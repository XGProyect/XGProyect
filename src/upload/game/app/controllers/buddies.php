...
class Buddies extends BaseController
{
    ...
    private function execAction($action)
    {
        try {
            ...
            $this->{$action}();
            Functions::redirect('index.php?page=buddies');
        } catch (Exception $e) {
    ...
    private function sendRequest()
    {
        ...
        if (!is_null($buddy) && $buddy->getBuddyId() != 0) {
            Functions::message($this->langs->line('bu_request_exists'), 'index.php?page=buddies', 3, true);
        }
    ...
    private function buildRequestForm()
    {
        ...
        if ($user == $this->user['user_id']) {
            Functions::message($this->langs->line('bu_cannot_request_yourself'), 'index.php?page=buddies', 2, true);
        }
    ...
    {
        return '<a href="index.php?page=buddies&mode=1&sm=' . $sm . '&bid=' . $buddy_id . '">' . $lang_line . '</a>';
    }
