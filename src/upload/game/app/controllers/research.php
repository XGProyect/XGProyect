...
class Research extends BaseController
{
    ...
    public function __construct()
    {
        ...
        // load models
        parent::loadModel('research');

        // load languages
        parent::loadLang(['global', 'research', 'technologies']);
                ...
                if (!$this->_is_working['is_working']) {
                    if (DevelopmentsLib::isDevelopmentPayable($this->user, $this->planet, $tech) && !$this->userLibrary->isOnVacations($this->user)) {
                        if (!$this->isLaboratoryInQueue()) {
                            ...
                            $action_link = UrlHelper::setUrl('index.php?page=research&cmd=search&tech=' . $tech, FormatLib::colorGreen($this->langs->line('re_research')));
                        }
                    } else {
                        $action_link = FormatLib::colorRed($this->langs->line('re_research'));
                    }
    ...
    private function doCommand()
    {
        ...
        if (!is_null($cmd)) {
            ...
            Functions::redirect('index.php?page=research');
        }
    }
