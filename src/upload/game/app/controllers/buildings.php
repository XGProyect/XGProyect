...
class Buildings extends BaseController
{
    ...
    public function __construct()
    {
        ...
        // load models
        parent::loadModel('buildings');

        // load languages
        parent::loadLang(['global', 'buildings', 'constructions']);
        ...
        if (!is_null($action)) {
            if (in_array($action, $allowed_actions)) {
                ...
                if ($reload == 'overview') {
                    ...
                    header('location:index.php?page=' . $this->getCurrentPage());
                }
            }
        }
    }
    ...
    {
        $build_url = 'index.php?page=' . $this->getCurrentPage() . '&cmd=insert&building=' . $building_id;
    ...
    private function removeFromQueue($QueueID)
    {
        if ($QueueID > 1) {
            ...
            if (!empty($CurrentQueue)) {
                ...
                $ActualCount = count($QueueArray);
                if ($ActualCount < 2) {
                    Functions::redirect('index.php?page=' . $this->getCurrentPage());
                }
                ...
                if ($BuildEndTime >= $CurrentTime) {
                    ...
                    if ($ListID > 0) {
                        ...
                        if ($ListID == 1) {
                            ...
                            $ListIDRow .= "		<a href=\"index.php?page=" . $this->getCurrentPage() . "&listid=" . $ListID . "&amp;cmd=cancel&amp;planet=" . $PlanetID . "\">" . $this->langs->line('bd_interrupt') . "</a></div>";
                            ...
                            $ListIDRow .= "		<a href=\"index.php?page=" . $this->getCurrentPage() . "&listid=" . $ListID . "&amp;cmd=remove&amp;planet=" . $PlanetID . "\">" . $this->langs->line('bd_cancel') . "</a></font>";
                        }
