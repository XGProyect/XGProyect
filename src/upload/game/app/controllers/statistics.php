...
class Statistics extends BaseController
{
    ...
    public function __construct()
    {
        ...
        // load models
        parent::loadModel('statistics');

        // load languages
        parent::loadLang(['global', 'statistics']);
    }
    ...
    private function buildPage()
    {
        ...
        if ($who == 2) {
            ...
            $parse['range'] = $this->buildRangeList($MaxAllys, $range);
            ...
            foreach ($query as $StatRow) {
                ...
                $parse['ally_rankplus'] = $this->rankDifference($ranking);
                $parse['ally_id'] = $StatRow['alliance_id'];
                $parse['alliance_name'] = $StatRow['alliance_name'];
                $parse['ally_members'] = $StatRow['ally_members'];
                $parse['ally_action'] = $StatRow['alliance_request_notallow'] == 1 ? '<a href="index.php?page=alliance&mode=apply&allyid=' . $StatRow['alliance_id'] . '"><img src="' . DPATH . 'img/m.gif" border="0" title="' . $this->langs->line('st_ally_request') . '" /></a>' : '';
            ...
            $parse['range'] = $this->buildRangeList($this->planet['stats_users'], $range);
            $parse['stat_header'] = $this->template->set(
            ...
            foreach ($query as $StatRow) {
                ...
                if ($StatRow['user_id'] != $this->user['user_id']) {
                    $parse['player_mes'] = '<a href="index.php?page=chat&playerId=' . $StatRow['user_id'] . '"><img src="' . DPATH . 'img/m.gif" border="0" title="' . $this->langs->line('write_message') . '" /></a>';
                } else {
                    $parse['player_mes'] = "";
                }
                ...
                if ($StatRow['alliance_name'] != '') {
                    if ($StatRow['alliance_name'] == $this->user['alliance_name']) {
                        $parse['player_alliance'] = '<a href="index.php?page=alliance&mode=ainfo&allyid=' . $StatRow['user_ally_id'] . '"><font color="#33CCFF">[' . $StatRow['alliance_name'] . ']</font></a>';
                    } else {
                        $parse['player_alliance'] = '<a href="index.php?page=alliance&mode=ainfo&allyid=' . $StatRow['user_ally_id'] . '">[' . $StatRow['alliance_name'] . ']</a>';
                    }
                } else {
                    $parse['player_alliance'] = '';
                }

                $parse['player_rankplus'] = $this->rankDifference($ranking);
    ...
    /**
     * method buildRangeList
     * param $count
     * param $range
     * return the list of range values
     */
    private function buildRangeList($count, $range)
    {
        $range_list = '';
        $last_page = 0;
     ...
     */
    private function rankingType($type)
    {
        // SWITCH TYPE
        switch ($type) {
            case 1: // TOTAL POINTS
