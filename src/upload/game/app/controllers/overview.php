...
class Overview extends BaseController
{
    ...
    public function __construct()
    {
        ...
        // load models
        parent::loadModel('overview');

        // load languages
        parent::loadLang(['global', 'overview', 'buildings', 'constructions']);
    ...
    private function getMessages()
    {
        ...
        if ($this->user['new_message'] != 0) {
            ...
            if ($this->user['new_message'] == 1) {
                $new_message .= '<th colspan="4">' . UrlHelper::setUrl('index.php?page=messages', $this->langs->line('ov_have_new_message'), $this->langs->line('ov_have_new_message')) . '</th>';
            }

            if ($this->user['new_message'] > 1) {
                $link_text = str_replace('%m', FormatLib::prettyNumber($this->user['new_message']), $this->langs->line('ov_have_new_messages'));
                $new_message .= '<th colspan="4">' . UrlHelper::setUrl('index.php?page=messages', $link_text, $link_text) . '</th>';
            }
    ...
    private function getPlanetMoon()
    {
        ...
        if ($this->planet['moon_id'] != 0 && $this->planet['moon_destroyed'] == 0 && $this->planet['planet_type'] == PlanetTypesEnumerator::PLANET) {
            ...
            $url = 'index.php?page=overview&cp=' . $this->planet['moon_id'] . '&re=0';
    ...
    private function getPlanets()
    {
        ...
        foreach ($planets_query as $user_planet) {
            if ($user_planet['planet_id'] != $this->user['user_current_planet'] && $user_planet['planet_type'] != PlanetTypesEnumerator::MOON) {
                $url = 'index.php?page=overview&cp=' . $user_planet['planet_id'] . '&re=0';
    ...
    private function getUserRank()
    {
        ...
        if ($this->_noob->isRankVisible($this->user['user_authlevel'])) {
            $user_rank = FormatLib::prettyNumber($this->user['user_statistic_total_points']) . " (" . $this->langs->line('ov_place') . ' ' . UrlHelper::setUrl('index.php?page=statistics&range=' . $total_rank, $total_rank, $total_rank) . ' ' . $this->langs->line('ov_of') . ' ' . $this->planet['stats_users'] . ")";
        }
