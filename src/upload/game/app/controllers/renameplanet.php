...
class Renameplanet extends BaseController
{
    ...
    private function buildPage(): void
    {
        ...
        switch ((isset($_POST['action']) ? $_POST['action'] : null)) {
            case $this->langs->line('rp_planet_rename_action'):
                $this->renamePlanet($_POST['newname']);
                break;
     ...
     */
    private function renamePlanet($new_name)
    {
        $new_name = strip_tags(trim($new_name));

        if (preg_match("/[^A-z0-9_\- ]/", $new_name) == 1) {
            Functions::message($this->langs->line('rp_newname_error'), "index.php?page=renameplanet", 2);
        }

        if ($new_name != '') {
            $this->Renameplanet_Model->updatePlanetName($new_name, $this->user['user_current_planet']);
            Functions::message($this->langs->line('rp_planet_name_changed'), "index.php?page=renameplanet", 2);
        }
    }
    ...
    private function deletePlanet()
    {
        ...
        if ($own_fleet > 0) {
            Functions::message($this->langs->line('rp_abandon_planet_not_possible'), 'index.php?page=renameplanet');
            ...
            if (password_verify($_POST['pw'], $this->user['user_password']) && $this->user['user_home_planet_id'] != $this->user['user_current_planet']) {
                ...
                Functions::message($this->langs->line('rp_planet_abandoned'), 'index.php?page=overview');
            } elseif ($this->user['user_home_planet_id'] == $this->user['user_current_planet']) {
                Functions::message($this->langs->line('rp_principal_planet_cant_abanone'), 'index.php?page=renameplanet');
            } else {
                Functions::message($this->langs->line('rp_wrong_pass'), 'index.php?page=renameplanet');
            }
        }
    }
}
