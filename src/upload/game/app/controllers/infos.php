...
class Infos extends BaseController
{
    ...
    public function __construct()
    {
        ...
        // load models
        parent::loadModel('infos');

        // load languages
        parent::loadLang(['global', 'infos', 'constructions', 'defenses', 'ships', 'technologies']);
    ...
    private function buildPage()
    {
        if (!array_key_exists($this->_element_id, $this->_resource)) {
            Functions::redirect('index.php?page=techtree');
        }
        ...
        if ($this->_element_id >= 1 && $this->_element_id <= 3) {
            ...
            if ($_POST) {
                Functions::message($this->doFleetJump(), "index.php?page=infos&gid=43", 2);
            }
        ...
        if ($TableHeadTPL != '') {
            ...
            if ($this->_element_id >= 22 && $this->_element_id <= 24) {
                ...
                $parse['table_data'] = $this->astrophysicsTable($TableTPL);
            } elseif ($this->_element_id == 42) {
        ...
        if ($GateTPL != '') {
            if ($this->planet[$this->_resource[$this->_element_id]] > 0) {
                ...
                $parse['gate_start_link'] = $this->planetLink($this->planet);
     ...
     */
    private function storageTable($template)
    {
    ...
    /**
     * method astrophysicsTable
     * param
     * return builds the astrophysics table
     */
    private function astrophysicsTable($template)
    {
     ...
     */
    private function planetLink($current_planet)
    {
        return "<a href=\"index.php?page=galaxy&mode=3&galaxy=" . $current_planet['planet_galaxy'] . "&system=" . $current_planet['planet_system'] . "\">[" . $current_planet['planet_galaxy'] . ":" . $current_planet['planet_system'] . ":" . $current_planet['planet_planet'] . "]</a>";
    }
    ...
    private function buildTearDownBlock(): string
    {
        ...
        if (isset($this->planet[$this->_resource[$this->_element_id]]) && $this->planet[$this->_resource[$this->_element_id]] > 0) {
            ...
            $tear_down_url = 'index.php?page=' . DevelopmentsLib::setBuildingPage($this->_element_id) . '&cmd=destroy&building=' . $this->_element_id;
