...
class Resources extends BaseController
{
    ...
    public function __construct()
    {
        ...
        // load models
        parent::loadModel('resources');

        // load languages
        parent::loadLang(['global', 'constructions', 'ships', 'resources']);
    ...
    private function buildPage()
    {
        ...
        foreach ($this->_reslist['prod'] as $ProdID) {
            if ($this->planet[$this->_resource[$ProdID]] > 0 && isset($this->_prod_grid[$ProdID])) {
                ...
                $CurrRow['option'] = $this->buildOptions($CurrRow['percent']);
        ...
        $parse['production_level'] = $this->prodLevel($this->planet['planet_energy_used'], $this->planet['planet_energy_max']);
        ...
        $parse['planet_metal_max'] = $this->resourceColor($this->planet['planet_metal'], $this->planet['planet_metal_max']);
        $parse['planet_crystal_max'] = $this->resourceColor($this->planet['planet_crystal'], $this->planet['planet_crystal_max']);
        $parse['planet_deuterium_max'] = $this->resourceColor($this->planet['planet_deuterium'], $this->planet['planet_deuterium_max']);
        ...
        $parse['daily_metal'] = $this->calculateDaily($this->planet['planet_metal_perhour'], $parse['production_level'], $parse['metal_basic_income']);
        $parse['weekly_metal'] = $this->calculateWeekly($this->planet['planet_metal_perhour'], $parse['production_level'], $parse['metal_basic_income']);

        $parse['daily_crystal'] = $this->calculateDaily($this->planet['planet_crystal_perhour'], $parse['production_level'], $parse['crystal_basic_income']);
        $parse['weekly_crystal'] = $this->calculateWeekly($this->planet['planet_crystal_perhour'], $parse['production_level'], $parse['crystal_basic_income']);

        $parse['daily_deuterium'] = $this->calculateDaily($this->planet['planet_deuterium_perhour'], $parse['production_level'], $parse['deuterium_basic_income']);
        $parse['weekly_deuterium'] = $this->calculateWeekly($this->planet['planet_deuterium_perhour'], $parse['production_level'], $parse['deuterium_basic_income']);
            ...
            foreach ($_POST as $Field => $Value) {
                ...
                if (isset($this->planet[$FieldName])) {
                    if (!in_array($Value, $ValidList['percent'])) {
                        Functions::redirect('index.php?page=resourceSettings');
                    }
     ...
     */
    private function buildOptions($current_porcentage)
    {
        $option_row = '';
     ...
     */
    private function prodLevel($energy_used, $energy_max)
    {
