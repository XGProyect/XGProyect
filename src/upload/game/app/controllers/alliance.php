...
class Alliance extends BaseController
{
    ...
    private function buildPage()
    {
        if (!$this->isPageAllowed()) {
            Functions::redirect('index.php?page=alliance');
        }
    ...
    private function getSearchSection()
    {
        ...
        if (!empty($search_string)) {
            ...
            foreach ($results->getAlliances() as $result) {
                $list_of_results[] = [
                    'ally_tag' => UrlHelper::setUrl('index.php?page=alliance&mode=apply&allyid=' . $result->getAllianceId(), $result->getAllianceTag()),
    ...
    private function getMakeSection()
    {
        ...
        if (is_array($action)) {
            ...
            if (strlen($alliance_tag) < 3 or strlen($alliance_tag) > 8) {
                Functions::message($this->langs->line('al_tag_required'), 'index.php?page=alliance&mode=make', 3);
            }

            if ($this->allianceTagExists($alliance_tag)) {
                Functions::message(strtr($this->langs->line('al_tag_already_exists'), ['%s' => $alliance_tag]), 'index.php?page=alliance&mode=make', 3);
            }

            if (strlen($alliance_name) < 3 or strlen($alliance_name) > 30) {
                Functions::message($this->langs->line('al_name_required'), 'index.php?page=alliance&mode=make', 3);
            }

            if ($this->allianceNameExists($alliance_name)) {
                Functions::message(strtr($this->langs->line('al_name_already_exists'), ['%s' => $alliance_name]), 'index.php?page=alliance&mode=make', 3);
            }
    ...
    private function getApplySection()
    {
        if (!$this->alliance->getCurrentAlliance()->getAllianceRequestNotAllow()) {
            Functions::message($this->langs->line('al_alliance_closed'), 'index.php?page=alliance', 3);
        }
        ...
        if (isset($request)) {
            if ($request['send'] != null && !empty($request['text'])) {
                ...
                Functions::message($this->langs->line('al_request_confirmation_message'), 'index.php?page=alliance', 3);
            }
        }
    ...
    private function getExitSection()
    {
        if ($this->alliance->isOwner()) {
            Functions::message($this->langs->line('al_founder_cant_leave_alliance'), 'index.php?page=alliance', 3);
        }
        ...
        return Functions::messageBox(
            ...
            '<br/>',
            'index.php?page=alliance&mode=exit&yes=1',
    ...
    private function getAdminAllySection()
    {
        ...
        if (isset($post['options'])) {
            ...
            Functions::redirect('index.php?page=alliance&mode=admin&edit=ally');
        }
        ...
        if (isset($post['t'])) {
            ...
            Functions::redirect('index.php?page=alliance&mode=admin&edit=ally&t=' . $t);
        }
    ...
    private function getAdminNameSection()
    {
        ...
        if (isset($name)) {
            if (strlen($name) < 3 or strlen($name) > 30) {
                Functions::message($this->langs->line('al_name_required'), 'index.php?page=alliance&mode=admin&edit=name', 3);
            }

            if ($this->allianceNameExists($name)) {
                Functions::message(strtr($this->langs->line('al_name_already_exists'), ['%s' => $name]), 'index.php?page=alliance&mode=admin&edit=name', 3);
            }
    ...
    private function getAdminRequestsSection()
    {
        ...
        if (isset($accept) && $show != 0) {
            ...
            Functions::redirect('index.php?page=alliance&mode=admin&edit=requests');
        }
    ...
    private function getAdminRightsSection()
    {
        ...
        if (is_array($ranks->getAllRanksAsArray())) {
            foreach ($ranks->getAllRanksAsArray() as $rank_id => $details) {
                $delete = '<a href="index.php?page=alliance&mode=admin&edit=rights&d=' . $rank_id . '"><img src="' . DPATH . 'alliance/abort.gif" border="0"></a>';
                $disabled = '';
    ...
    private function getAdminTagSection()
    {
        ...
        if (isset($tag)) {
            if (strlen($tag) < 3 or strlen($tag) > 8) {
                Functions::message($this->langs->line('al_tag_required'), 'index.php?page=alliance&mode=admin&edit=tag', 3);
            }

            if ($this->allianceTagExists($tag)) {
                Functions::message(strtr($this->langs->line('al_tag_already_exists'), ['%s' => $tag]), 'index.php?page=alliance&mode=admin&edit=tag', 3);
            }
    ...
    private function buildPublicRequestsBlock()
    {
        if (!$this->user['user_ally_id']
            ...
            $url = UrlHelper::setUrl(
                'index.php?page=alliance&mode=apply&allyid=' . $this->getAllianceId(),
    ...
    private function buildMembersBlock()
    {
        ...
        if ($this->alliance->hasAccess(AllianceRanks::VIEW_MEMBER_LIST)) {
            $list_of_members = ' (' . UrlHelper::setUrl('index.php?page=alliance&mode=memberslist', $this->langs->line('al_user_list')) . ')';
        }
    ...
    private function buildRankBlock()
    {
        ...
        if ($this->alliance->hasAccess(AllianceRanks::ADMINISTRATION)) {
            $admin_area = ' (' . UrlHelper::setUrl('index.php?page=alliance&mode=admin&edit=ally', $this->langs->line('al_manage_alliance')) . ')';
        }
    ...
    private function buildRequestsBlock()
    {
        ...
        if ($this->alliance->hasAccess(AllianceRanks::APPLICATION_MANAGEMENT) && $count != 0) {
            $requests = UrlHelper::setUrl(
                'index.php?page=alliance&mode=admin&edit=requests',
    ...
    private function buildCircularBlock()
    {
        if ($this->alliance->hasAccess(AllianceRanks::SEND_CIRCULAR)) {
            return [
                ...
                'detail_content' => UrlHelper::setUrl('index.php?page=alliance&mode=circular', $this->langs->line('al_send_circular_message')),
            ];
        }
    }
    ...
    private function buildAdminMembersActionBlock($member_id, $member_name, $requested_rank = 0)
    {
        ...
        if ($this->alliance->hasAccess(AllianceRanks::KICK)) {
            $action = 'index.php?page=alliance&mode=admin&edit=members&kick=' . $member_id;
        ...
        if ($this->alliance->hasAccess(AllianceRanks::ADMINISTRATION)) {
            $action = 'index.php?page=alliance&mode=admin&edit=members&rank=' . $member_id;
