...
function t() {
	...
	m           = 0;
	h           = 0;
	rurl        = "index.php?page={call_program}&planet=" + pl;
	if ( (ss + 3) < aa ) {
		...
		if ( s <= 0 ) {
			...
			blc.innerHTML = h + ":" + m + ":" + s + "<br><a href=index.php?page={current_page}&r={call_program}&listid=" + pk + "&cmd=" + pm + "&planet=" + pl + ">{bd_cancel}</a>";
		}
		pp = pp - 1;
		if (timeout == 1) {
