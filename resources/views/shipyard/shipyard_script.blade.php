<br>
{{ __('game/shipyard.sy_actual_production') }}
<div id="bx" class="z"></div>
	...
	b  = new Array({!! $b !!}'');
	a  = new Array({{ $a }}'');
	aa = '{{ __('game/shipyard.sy_completed') }}';
	...
	function xd() {
		...
		for ( iv = p; iv <= b.length - 2; iv++ ) {
			...
			if ( iv == p ) {
				act = " {{ __('game/shipyard.sy_operating') }}";
			} else {
				act = "";
			}
