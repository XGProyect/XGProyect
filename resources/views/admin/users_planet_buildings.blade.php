@php
    ...
    $save_route = $is_moon
        ? route('admin.users.moon.buildings.update', [$user->id, $planet_id])
        : route('admin.users.planet.buildings.update', [$user->id, $planet_id]);
    $edit_route = $is_moon
                        ...
                        <div class="row">
                            @foreach ($buildings as $b)
                                <div class="col-md-4 col-lg-3 mb-3">
                                    <label class="small font-weight-bold text-gray-700" for="{{ $b['field'] }}">
                                        {{ $b['label'] }}
                                    </label>
                                    <input type="number" id="{{ $b['field'] }}" name="{{ $b['field'] }}"
                                        class="form-control form-control-sm"
                                        min="0" value="{{ old($b['field'], $b['level']) }}">
                                </div>
                            @endforeach
                        </div>
