@php
    ...
    $save_route = $is_moon
        ? route('admin.users.moon.defenses.update', [$user->id, $planet_id])
        : route('admin.users.planet.defenses.update', [$user->id, $planet_id]);
    $edit_route = $is_moon
    ...
    $ships_route = $is_moon
        ? route('admin.users.moon.ships', [$user->id, $planet_id])
        : route('admin.users.planet.ships', [$user->id, $planet_id]);
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="mb-3 d-flex flex-wrap" style="gap: 0.4rem;">
                ...
                <a class="btn btn-sm btn-secondary btn-icon-split" href="{{ $ships_route }}">
            ...
            <x-admin.card
                    title="{{ __('admin/users.us_defenses_title') }}"
                        ...
                        <div class="row">
                            @foreach ($defenses as $d)
                                <div class="col-md-4 col-lg-3 mb-3">
                                    <label class="small font-weight-bold text-gray-700" for="{{ $d['field'] }}">
                                        {{ $d['label'] }}
                                    </label>
                                    <input type="number" id="{{ $d['field'] }}" name="{{ $d['field'] }}"
                                        class="form-control"
                                        min="0" value="{{ old($d['field'], $d['amount']) }}">
                                </div>
                            @endforeach
                        </div>
